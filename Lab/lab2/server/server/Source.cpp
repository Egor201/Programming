...
const string webhook_template = u8R"(
                    <div class="form-row align-items-center">
                      ...
                      <div class="col">
                        <button type="submit" name="del" value="https://webhook.site/4fab2dd1-e7d4-4d95-b9b9-36883b6b7e4b" class="btn btn-danger mb-2">Удалить</button>
                      </div>
                    </div>)";
...
void alice(const Request& req, Response& res)
{
	...
	if (request["session"]["new"].get<bool>())
	{
		...
		json response = gen_response(
			text = u8"Здравствуйте",
			tts = u8"Здр+авствуйте",
			exit_button,
			session_now);
	...
	else
	{
		...
		else if (command.find(u8"удалить из корзины") == 0)
		{
			...
			if (item_name == "")
			{
				text = u8"что собираетесь убирать.";
				tts = u8"что собир+аетесь убир+ать.";
			}
			else
			{
				item_name.pop_back();
				...
				if (!found_item)
				{
					text = u8"Такого в вашей корзине нет";
					tts = u8"Так+ого в в+ашей корз+ине н+ет.";
				}
				else
				{
					text = u8"Галя, отмена!";
